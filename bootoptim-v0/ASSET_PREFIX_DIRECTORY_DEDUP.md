...
Pandora stores an ordinary asset object at `<assets objects>/<first two SHA-1 hex chars>/<full SHA-1>`. The stock loop decodes the full SHA-1, calls `std::fs::create_dir(prefix)` while ignoring the result, and then schedules the unchanged SHA-1 verification/download task. For an index with thousands of objects this repeats a directory-creation syscall for the same at-most-256 prefix directories.

## Candidate

The loop keeps a process-local, per-call 256-entry readiness bitmap indexed by the already-decoded first SHA-1 byte. For each object, hash decoding still happens first and returns the same `InvalidHash` error on malformed input. A prefix is marked ready only when:

- `create_dir` succeeds; or
- `create_dir` reports `AlreadyExists` and the path is confirmed to be a directory.

Once marked ready, later objects with that prefix skip the redundant creation attempt. No persistent state or cache is added.
...
- an already-existing prefix (`AlreadyExists` + directory) and deduplication thereafter;
