...
This is narrower than the Java Properties grammar on purpose. OpenJDK 21 `Properties.load` skips comments and blank lines and builds key/value mappings, while `Properties.store` writes a generated date comment and ordinary `Properties` entries in key order. Unsupported legal Java Properties forms are therefore intentionally raw rather than approximately parsed:
