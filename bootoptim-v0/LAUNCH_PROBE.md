...
5. `version_loader_resolution` — Minecraft/loader launch-version resolution. Forge/NeoForge can create Java/library verifier trackers while constructing the launch version; those completed nested trackers are not mistaken for the later top-level group. In this pinned Pandora revision, the Forge/NeoForge progress shape is also the path that immediately attempts the remote installer SHA-1 request, so the probe emits `network_request source="loader_sha1"` once for that route. This is an instantaneous route observation, not a network-duration estimate.
6. `java_runtime`, `assets_verify_download`, `libraries_classpath_inputs` — the observable top-level concurrent preparation branches in Pandora's existing `try_join4`. `load_assets` is not used during Forge/NeoForge launch-version construction, so creation of the asset tracker establishes that the outer group has begun; any still-live Java/library tracker at that point belongs to the same outer group. This avoids depending on fragile progress-count totals for phase attribution and also tolerates a custom Java path with no Java-runtime tracker.
