...
- `network`: `true` only for the fixed `network_download` observation emitted when an existing Java-runtime/assets/libraries tracker enters Pandora's explicit download path.
...
5. `version_loader_resolution` — Minecraft/loader launch-version resolution. Forge/NeoForge can create Java/library verifier trackers while constructing the launch version; those completed nested trackers are not mistaken for the later top-level group.
6. `java_runtime`, `assets_verify_download`, `libraries_classpath_inputs` — the observable top-level concurrent preparation branches in Pandora's existing `try_join4`. `load_assets` is not used during Forge/NeoForge launch-version construction, so creation of the asset tracker establishes that the outer group has begun; any still-live Java/library tracker at that point belongs to the same outer group. This avoids depending on fragile progress-count totals and also tolerates a custom Java path with no Java-runtime tracker.
7. `network_download` — an instantaneous observation with fixed `source` when one of those existing verifier trackers switches to Pandora's explicit download path. It is not a claimed network-duration span. A long asset span with no `source="assets"` observation is therefore evidence that the measured asset time was local verification rather than asset download. The probe does not claim to intercept every small authentication/metadata HTTP request outside these tracker-backed downloader paths.
8. `classpath_resolution`, `native_extraction`, and `wrapper_arguments` — deliberately **inclusive, overlapping upper-bound envelopes**. They begin once all observable top-level verifier siblings have finished and end when Pandora has constructed the final structured Minecraft command. Because the fourth `try_join4` sibling (`log_configuration`) has no progress tracker in this Pandora revision, the envelope may include a short untracked log-config tail before the real post-join work. This is intentionally conservative; do not treat these scopes as exclusive timings.
9. `appcds_preflight` — optional span emitted only when the existing BootOptim v0 helper actually reaches its preflight subprocess. It measures helper execution separately from wrapper preparation and Java spawn without changing AppCDS decisions.
10. `java_spawn` — direct Java OS process-creation begin/end on Pandora's command-spawner thread. It is emitted only when the direct executable is `java`, `java.exe`, or `javaw.exe` and the Pandora LaunchWrapper marker is present. A custom external wrapper is therefore not falsely labelled as Java creation.
11. `launcher_pre_java` — successful end boundary for Start→direct Java process creation. Compute the duration as `launcher_pre_java.end.mono_ns - launch_request.mono_ns`; do not add child spans because several overlap.
12. `java_to_menu` — emitted as `unobserved`, `observed=false`, `duration_ns=null` immediately after successful direct Java creation. Pandora has no menu-ready signal usable here without reading/interpreting game output, and this PR intentionally does not read game logs to manufacture a TTMM value.
...
### Nested Forge/NeoForge trackers

The exact Forge/NeoForge path may verify Java and installer libraries while `create_launch_version` is still running. The probe does not close `version_loader_resolution` merely because such a tracker appears. The outer asset tracker is the discriminator: assets are only loaded after launch-version construction. When that tracker appears, completed Java/library trackers remain nested; any still-live Java/library trackers are promoted to the outer parallel group using their already captured monotonic creation timestamps.
...
Source inspection also shows that Forge/NeoForge launch-version construction performs a remote installer-SHA1 request inside `version_loader_resolution`. That is a separate optimization hypothesis only; this PR does not cache, eliminate, reorder, or falsely time it as an asset/library download.
