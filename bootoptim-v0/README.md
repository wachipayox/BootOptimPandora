...
Use the Windows Actions artifact **only after its Windows build/test job is green and `SHA256SUMS.txt` has been verified from that artifact**. Do not modify the productive modpack for mutation tests; those identity-change cases are automated in unit tests.
