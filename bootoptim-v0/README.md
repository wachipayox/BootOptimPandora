...
- `GENERATING`: persisted while a training result is pending. A concurrent/early launch that sees `training.meta` but no completed `training.jsa` launches stock and does not start another training write.
- `READY`: on a later exact preflight, a non-empty `training.jsa` with matching training metadata is promoted to `ready.jsa` plus hashed `ready.meta`; the same launch may then consume it with `-Xshare:auto -XX:SharedArchiveFile=<ready>`.
...
- `FAILED`: malformed/orphan cache state, conflicting staging, or ineligible identity. Launch stock. v0 favors inspection/manual reset over silently rewriting ambiguous state.
...
The training file is produced by the exact Java process at VM exit. The helper does not keep a proxy process alive to observe that exit. Consequently `state.meta` remains `GENERATING` after the training process exits and changes to `READY` only on the next exact preflight that can see and promote the completed training file. This preserves Pandora's direct ownership of Java and its stop/kill semantics.
...
5. Only after the two-plan gate passes, set `$env:BOOTOPTIM_APPCDS_MODE = 'auto'` and launch once. This is the **training/generation run**, not a performance result. Pandora should log `BOOTOPTIM_INTERPOSER status=generating activation=training`. Exit Minecraft normally after reaching the intended menu. After exit, require `training.meta` and a non-empty `training.jsa`; `state.meta` is expected to remain `GENERATING` until the next preflight.
6. Launch once more with the exact same tuple and auto mode. Its preflight must promote the completed training archive, then Pandora should log `BOOTOPTIM_INTERPOSER status=ready activation=enabled` and directly spawn Java with the ready archive. Require `state.meta=READY`, `ready.jsa`, and `ready.meta`; `training.meta`/`training.jsa` should be gone. Only this run is a candidate archive-consumption observation. Do not infer a startup saving from generation or from this single consumption run.
