...
- `FAILED`: malformed/orphan training state, corrupt training metadata/completion, conflicting staging, a completion marker without a usable archive, or ineligible identity. Launch stock. Ambiguous training files are invalidated/isolated rather than promoted. A `training.invalid` campaign stays stock-only until the instance-local AppCDS cache is deliberately reset; it is not auto-cleared while an old Java process could still write canonical training paths.
...
5. If the later identity differs, or if training metadata/completion/archive state is malformed, the helper must return `STOCK`, must not create/use `ready.jsa`, and must persist `training.invalid` before best-effort cleanup. That tombstoned campaign remains stock-only until a deliberate instance-local AppCDS cache reset; this prevents a still-running/late-exiting old Java process from writing `training.jsa` or `training.complete` into a newly trusted campaign. Do not weaken the identity inputs to force a match.
