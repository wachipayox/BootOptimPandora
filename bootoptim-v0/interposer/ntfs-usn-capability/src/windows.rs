...
use std::{ffi::{c_void, OsStr}, fs::{File, OpenOptions}, io::{Seek, SeekFrom, Write}, mem::zeroed, os::windows::{ffi::OsStrExt, fs::OpenOptionsExt, io::AsRawHandle}, path::Path, ptr::{null, null_mut}, time::{Duration, Instant}};
...
fn write_exact(h:Handle,i:&[u8])->bool{let mut w=0;let ok=unsafe{WriteFile(h,i.as_ptr().cast(),i.len() as u32,&mut w,null_mut())};ok!=0&&w as usize==i.len()}
