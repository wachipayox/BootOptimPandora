...
fn persist_plan_and_compare(cache_dir: &Path, bytes: &[u8], sha: &str) -> io::Result<bool> {
    ...
    let old_plan = fs::read(&plan_path).ok();
    let stable = old_plan.as_deref().map(|old| old == bytes).unwrap_or(false);
    if !stable && env::var_os(APPCDS_IDENTITY_DIAGNOSTICS_ENV).is_some_and(|value| value == "1") {
        let changed = match old_plan.as_deref() {
            None => "no-previous-plan".to_string(),
            Some(old) => changed_plan_fields(old, bytes).unwrap_or_else(|| "unparsed".to_string()),
        };
        eprintln!("BOOTOPTIM_APPCDS_PLAN_DIFF fields={changed}");
    }
...
fn changed_plan_fields(old: &[u8], current: &[u8]) -> Option<String> {
    let old_fields = top_level_json_fields(old)?;
    let current_fields = top_level_json_fields(current)?;
    let mut changed = old_fields
        .keys()
        .chain(current_fields.keys())
        .filter(|key| old_fields.get(*key) != current_fields.get(*key))
        .cloned()
        .collect::<Vec<_>>();
    changed.sort();
    changed.dedup();
...
// The launch plan is generated by this binary with canonical top-level JSON fields.
// This small scanner compares those fields without deserializing or logging their values.
fn top_level_json_fields(input: &[u8]) -> Option<BTreeMap<String, Vec<u8>>> {
    fn skip_ws(input: &[u8], index: &mut usize) {
        while input.get(*index).is_some_and(|byte| byte.is_ascii_whitespace()) {
            *index += 1;
        }
    }

    fn parse_string(input: &[u8], index: &mut usize) -> Option<String> {
        ...
        *index += 1;
        ...
        while let Some(&byte) = input.get(*index) {
            if escaped {
                ...
                let value = std::str::from_utf8(&input[start..*index]).ok()?;
                *index += 1;
    ...
    loop {
        ...
        let key = parse_string(input, &mut index)?;
        skip_ws(input, &mut index);
        if input.get(index) != Some(&b':') {
            return None;
        }
        ...
        let value_end = index;
        let value = std::str::from_utf8(&input[value_start..value_end]).ok()?.trim_end();
        fields.insert(key, value.as_bytes().to_vec());
...
#[cfg(test)]
mod appcds_plan_diagnostics_tests {
    use super::*;
    ...
    #[test]
    fn plan_diff_rejects_malformed_json_instead_of_guessing() {
        assert_eq!(top_level_json_fields(b"not json"), None);
        assert_eq!(changed_plan_fields(b"{", b"{}"), None);
    }
}
