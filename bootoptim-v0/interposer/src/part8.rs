...
mod appcds_identity_cache_tests {
    ...
    #[test] fn every_capability_failure_is_stock_only(){let c=cached();for failure in [UsnCapabilityFailure::HelperMissing,UsnCapabilityFailure::HelperIdentityMismatch,UsnCapabilityFailure::UacDenied,UsnCapabilityFailure::HelperCrash,UsnCapabilityFailure::Timeout,UsnCapabilityFailure::MalformedProtocol,UsnCapabilityFailure::InvalidPipeAcl,UsnCapabilityFailure::InvalidPeerPid,UsnCapabilityFailure::NonNtfs,UsnCapabilityFailure::ReparsePoint,UsnCapabilityFailure::NotRegularFile,UsnCapabilityFailure::ProtectedHandleUnavailable,UsnCapabilityFailure::Io]{let mut e=evidence();e.capability=Err(failure);assert_eq!(evaluate_identity_reuse(&c,&e),Err(IdentityMiss::Capability));}}
