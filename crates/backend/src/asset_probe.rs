...
impl AssetAttributionProbe {
    ...
    fn hash_file_impl(&self, file: &mut File, expected_hash: [u8; 20]) -> io::Result<(bool, u64)> {
        ...
        Ok((expected_hash == actual_hash, bytes_read))
    }

    fn record_hash_result(&self, result: io::Result<(bool, u64)>) -> bool {
    ...
    pub(crate) fn hash_path(&self, path: &Path, expected_hash: [u8; 20]) -> bool {
        self.begin_hash();
        let result = File::open(path).and_then(|mut file| self.hash_file_impl(&mut file, expected_hash));
        self.record_hash_result(result)
    }

    /// Hash an already-protected file handle without reopening the pathname.
    /// This is the composition seam for PR #11: attribution observes the exact
    /// same-handle SHA-1 fallback while leaving the USN reuse decision untouched.
    #[cfg(windows)]
    pub(crate) fn hash_open_file(&self, file: &mut File, expected_hash: [u8; 20]) -> bool {
        self.begin_hash();
        let result = self.hash_file_impl(file, expected_hash);
        self.record_hash_result(result)
    }
...
mod tests {
    ...
    #[test]
    fn asset_attribution_sidecar_is_create_new() {
        let out_path = temp_path("create-new");
        let _ = std::fs::remove_file(&out_path);

        let first = AssetAttributionProbe::from_configured_path(Some(out_path.clone()), 0, 0, true).unwrap();
        first.finish("ok");
        let original = std::fs::read(&out_path).unwrap();

        let second = AssetAttributionProbe::from_configured_path(Some(out_path.clone()), 0, 0, true).unwrap();
        second.finish("error");
        assert_eq!(std::fs::read(&out_path).unwrap(), original);
        assert_eq!(read_snapshot(&out_path)["outcome"], "ok");
