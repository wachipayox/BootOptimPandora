...
use bridge::{
    ...
    instance::{ContentType, InstanceContentSummary, InstanceID},
...
use schema::{
    ...
    curseforge::{
        CurseforgeChangelogRequest, CurseforgeChangelogResult, CurseforgeFile, CurseforgeGetModFilesRequest,
        CurseforgeGetModFilesResult, CurseforgeModLoaderType, CurseforgeReleaseType,
    },
    instance::UpdateChannel,
    loader::Loader,
    modrinth::{
        ModrinthChangelogRequest, ModrinthChangelogResult, ModrinthLoader, ModrinthProjectVersion,
        ModrinthProjectVersionsRequest, ModrinthProjectVersionsResult, ModrinthVersionType,
    },
};
use ustr::Ustr;
        ...
        ContentSource::ModrinthProject { project_id } => {
            MetadataRequest::ModrinthProjectVersions(ModrinthProjectVersionsRequest {
                ...
                game_versions: minecraft_version
                    .map(|minecraft_version| vec![Arc::<str>::from(minecraft_version.as_str())].into()),
...
fn is_release(item: &VersionItem) -> bool {
    match &item.source {
        ...
        VersionSource::Curseforge(file) => {
            CurseforgeReleaseType::from_u32(file.release_type) == CurseforgeReleaseType::Release
        },
    }
}
...
fn is_beta(item: &VersionItem) -> bool {
    match &item.source {
        ...
        VersionSource::Curseforge(file) => {
            CurseforgeReleaseType::from_u32(file.release_type) == CurseforgeReleaseType::Beta
        },
    }
}
...
fn is_alpha(item: &VersionItem) -> bool {
    match &item.source {
        ...
        VersionSource::Curseforge(file) => {
            CurseforgeReleaseType::from_u32(file.release_type) == CurseforgeReleaseType::Alpha
        },
    }
}
...
impl ChangeVersionDialog {
    ...
    fn request_versions(&mut self, cx: &mut Context<Self>) {
        ...
        let request = if self.show_incompatible {
            ...
            versions_request(
                ...
                Some(self.modrinth_loaders.clone()),
    ...
    fn process_changelog_metadata(
        &mut self,
        key: &VersionKey,
        changelog: Entity<FrontendMetadataState>,
        cx: &mut Context<Self>,
    ) {
        if self.current_changelog.as_ref() != Some(key) {
            return;
    ...
    fn render_version_select(&mut self, window: &mut Window, cx: &mut Context<Self>) -> AnyElement {
        ...
        let state = if let Some(state) = self.version_select_state.clone() {
            ...
            self.current_date_published = items
                .iter()
            ...
            let preselected = match self.update_channel {
                UpdateChannel::Release => items
                    .iter()
                    .position(is_release)
                    .or_else(|| items.iter().position(is_beta))
                    .or_else(|| items.iter().position(is_alpha)),
                UpdateChannel::Beta => items
                    .iter()
                    .position(|item| is_release(item) || is_beta(item))
                    .or_else(|| items.iter().position(is_alpha)),
    ...
    fn render_changelog_area(&mut self, selected_key: Option<&VersionKey>, cx: &mut Context<Self>) -> AnyElement {
        ...
        if let Some(error) = self.changelog_error.clone() {
            return ErrorAlert::new(t::instance::content::change_version::error_loading_changelog().into(), error)
                .into_any_element();
        }
        ...
        let body = match display {
            ChangelogDisplay::Loading => h_flex()
                ...
                .child(t::instance::content::change_version::loading_changelog())
                .into_any_element(),
            ChangelogDisplay::Loaded(None) => h_flex()
                ...
                .justify_center()
                .text_color(cx.theme().muted_foreground)
                .child(t::instance::content::change_version::no_changelog())
                .into_any_element(),
            ChangelogDisplay::Loaded(Some(ChangelogContent::Markdown(changelog))) => {
                TextView::markdown("changelog", changelog)
                    .scrollable(true)
                    .h_full()
                    .w_full()
                    .into_any_element()
    ...
    fn render(&mut self, modal: Dialog, window: &mut Window, cx: &mut Context<Self>) -> Dialog {
        ...
        if let Some(error) = self.versions_error.clone() {
            return modal.child(v_flex().gap_3().child(ErrorAlert::new(
                t::instance::content::change_version::error_loading_versions().into(),
                error,
            )));
        }
        ...
        let same_version =
            selected.as_ref().and_then(|selected| selected.sha1.as_deref()) == Some(self.installed_sha1.as_ref());

        let label: SharedString = match (&selected, is_downgrade) {
            (Some(selected), true) => {
                t::instance::content::change_version::downgrade_to(selected.raw_name.as_str()).into()
            },
            (Some(selected), false) => {
                t::instance::content::change_version::update_to(selected.raw_name.as_str()).into()
            },
        ...
        let action_button = Button::new("change_version_action")
            ...
            .when_some(selected.clone(), |button, selected| {
                button.on_click({
                    ...
                    move |_, window, cx| {
                        ...
                        let install_file = match &selected.source {
                            ...
                            VersionSource::Curseforge(file) => {
                                let Some(sha1) = file.hashes.iter().find(|hash| hash.algo == 1).map(|hash| &hash.value)
                                else {
                                ...
                                if hex::decode_to_slice(&**sha1, &mut hash).is_err() {
                                    let warning =
                                        t::instance::content::install::file_invalid_sha1(&file.file_name, sha1);
                                    window.push_notification((NotificationType::Error, warning), cx);
                                    return;
                                }
        ...
        let content = v_flex()
            ...
            .child(
                v_flex()
                    .gap_0p5()
                    .child(div().text_sm().text_color(cx.theme().muted_foreground).child(self.version_label.clone()))
